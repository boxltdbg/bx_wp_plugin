(function ($) {
    ...
    function updateLockerDetailsContainer(lockerData) {
      ...
      // If the language is not defined, default to Bulgarian.
  ...
  // Define the content for Bulgarian.
  ...
  <div style="font-family: Arial, sans-serif; margin-top: 10px;">
    ...
    function toggleBoxNowDeliveryButton() {
      // Check if the BoxNow delivery option is checked or if no other shipping option is selected
      if ($("#shipping_method_0_box_now_delivery").is(":checked") || !$('input[name="shipping_method[0]"]:checked').val()) {
        // Show the button and set its background color
        $("#box_now_delivery_button")
          .css("background-color", boxNowDeliverySettings.buttonColor)
          .show();
      } else {
        // If the BoxNow delivery option is not checked and another shipping option is selected, hide the button
