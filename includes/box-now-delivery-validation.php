...
class BNDP_Serializer
{
  ...
  public function boxnow_settings_save()
  {
    ...
    if (isset($_POST['boxnow_locker_not_selected_message'])) {
      update_option('boxnow__not_selected_message', sanitize_text_field($_POST['boxnow_locker_not_selected_message']));
    }

    // Save the Exclude Shipping Class option
    if (isset($_POST['boxnow_exclude_class'])) {
      update_option('boxnow_exclude_class', sanitize_text_field($_POST['boxnow_exclude_class']));
    }
