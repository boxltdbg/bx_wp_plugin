...
function boxnow_render_voucher_column($column, $order_or_id)
{
    ...
    foreach ($parcel_ids as $parcel_id) {
        $pdf_url = admin_url('admin-ajax.php?action=print_box_now_voucher&parcel_id=' . urlencode($parcel_id));
...
function boxnow_render_indicator_column($column, $order_or_id)
{
    ...
    echo '<span class="boxnow-order-indicator" title="' . esc_attr__('BOX NOW Delivery', 'boxnowbulgaria') . '">BOX NOW</span>';
}
...
function boxnow_filter_admin_order_number($order_number, $order)
{
    ...
    $badge = '<span class="boxnow-order-indicator" title="' . esc_attr__('BOX NOW Delivery', 'boxnowbulgaria') . '">BOX NOW</span> ';
